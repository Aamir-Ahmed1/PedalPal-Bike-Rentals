...
async function fetchBikes() {
  ...
  error.value = ''
  try {
    bikes.value = await api.mountainBikes()
  } catch {
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-brand-blue to-brand-cyan">
    <div class="max-w-6xl mx-auto px-4 py-8">
      <div class="mb-8 flex items-center gap-4">
        ...
        <div class="flex-1 text-center">
          ...
          <p class="text-white/70 mt-1">More gears than you'll use. Exactly the right amount of fun.</p>
        </div>
        <div class="w-20"></div>
      </div>
      ...
      <div v-else-if="bikes.length === 0" class="text-center py-20 text-white/70">
        No mountain bikes available right now.
      </div>
